...
while cap.isOpened():
    ...
    if len(faces) > 0:
        ...
        '''
        hist, bin_edges = np.histogram(left_eye_histeq, bins=256)
    
        plt.figure(figsize=(14, 5))
        plt.subplot(121)
        plt.imshow(left_eye_histeq, cmap='gray')
        ...
        cv2.waitKey()
        '''
        #left_eye_opened = cv2.morphologyEx(left_eye_histeq, cv2.MORPH_OPEN, SE)
        #right_eye_opened = cv2.morphologyEx(right_eye_histeq, cv2.MORPH_OPEN, SE)
        ...
        # 소벨 엣지 추출
        #left_eye_sobel = cv2.Sobel(left_eye_gray,cv2.CV_8U,1,0,3)
        #right_eye_sobel = cv2.Sobel(right_eye_gray,cv2.CV_8U,1,0,3)

        #left_eye_laplace =cv2.Laplacian(left_eye_es,cv2.CV_16S,5)
        left_eye_canny = cv2.dilate(cv2.Canny(left_eye_opened, 150, 255),SE)
        #left_eye_canny = cv2.morphologyEx(cv2.dilate(cv2.Canny(left_eye_histeq, 150, 255),SE), cv2.MORPH_OPEN, SE2)
        # right_eye_laplace = cv2.Laplacian(right_eye_es,cv2.CV_16S,5)
        right_eye_canny = cv2.dilate(cv2.Canny(right_eye_opened, 150, 255),SE)
        #right_eye_canny = cv2.morphologyEx(cv2.dilate(cv2.Canny(left_eye_histeq, 150, 255),SE), cv2.MORPH_OPEN, SE)
        ...
        result = cv2.seamlessClone(
            left_eye_opened,
            result,
            np.full(left_eye_opened.shape[:2], 255, left_eye_opened.dtype),
            (300, 400),
            cv2.NORMAL_CLONE
            #cv2.MIXED_CLONE
        )
        ...
        result = cv2.seamlessClone(
            right_eye_opened,
            result,
            np.full(right_eye_opened.shape[:2], 255, right_eye_opened.dtype),
            (455, 400),
            cv2.NORMAL_CLONE
            #cv2.MIXED_CLONE
        )
        ...
        #mouth_canny = cv2.dilate(cv2.Canny(mouth_histeq, 150, 255),SE)
        result = cv2.seamlessClone(
            mouth_opened,
            result,
            np.full(mouth_opened.shape[:2], 255, mouth_opened.dtype),
            (385, 550),
            cv2.NORMAL_CLONE
        )
